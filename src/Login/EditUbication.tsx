...
import { getAddress, updateAddress } from './Petitions';
import { AppContext } from '../Context/AppContext';
import { Address } from '../Model/Address';
import { RouteProp, useFocusEffect, useRoute } from '@react-navigation/native';
//import BackgroundGeolocation from 'react-native-background-geolocation';

type EditAddressRouteProps = RouteProp<{ params: { addressId: number } }, 'params'>;
export const EditUbication = ({ navigation }: any) => {
  const route = useRoute<EditAddressRouteProps>();
  const { addressId } = route.params; // Desestructurar el parámetro con fallback en caso de que sea undefined
  ...
  console.log('ID de la dirección:', addressId);
  ...
  useEffect(() => {
    ...
    getAddress(addressId).then(response => {
      if (response) {
        setHouseNumber(response.Location);
        setReferences(response.References);
      }
      context.setIsShowSplash(false); // Ocultamos el splash
    });
  }, [addressId]);
